...
export default function AIConsultantScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: '안녕하세요! 저는 AI 상담원입니다. 🤖\n수술을 망설이시는 이유를 말씀해주세요. 당신의 고민을 해결할 수 있는 후기를 찾아드릴게요!',
      sender: 'ai',
      timestamp: '09:00',
    }
  ]);
  ...
  const sendMessage = async () => {
    ...
    setTimeout(() => {
      const concerns = analyzeConcerns(newMessage.text);
      const reviews = findMatchingReviews(concerns);
      
      const aiResponse: Message = {
        id: messages.length + 2,
        text: `분석 완료! 당신의 고민: "${concerns}"\n\n이런 고민을 가진 분들의 후기를 찾았어요. 아래 후기들을 확인해보세요! ✨`,
        sender: 'ai',
        timestamp: new Date().toLocaleTimeString('ko-KR', { 
          hour: '2-digit', 
          minute: '2-digit' 
  ...
  const analyzeConcerns = (text: string): string => {
    // 실제로는 AI로 분석
    if (text.includes('자연스럽') || text.includes('티')) {
      return '자연스러운 결과';
    } else if (text.includes('붓기') || text.includes('회복')) {
      return '빠른 회복';
    } else if (text.includes('통증') || text.includes('아프')) {
      return '통증 최소화';
    } else if (text.includes('가격') || text.includes('비용')) {
      return '합리적인 가격';
    }
    return '안전한 수술';
  };

  const findMatchingReviews = (concern: string): RecommendedReview[] => {
    // 실제로는 DB에서 검색
    return [
      {
        id: 1,
    ...
    <View style={styles.container}>
      ...
      >
                ...
                >
                  <View style={styles.reviewHeader}>
                    ...
                    <View style={styles.similarityBadge}>
                      <Text style={styles.similarityText}>{review.similarity}%</Text>
                    </View>
                  </View>
        ...
        <View style={styles.inputContainer}>
          <TextInput
            style={styles.input}
            placeholder="고민을 입력하세요... (예: 자연스럽게 되고 싶어요)"
            value={message}
